...
const getStatusCounts = (allGroupsStatus) => {
    const statusCounts = {
        waiting: 0,
        accept: 0,
        reject: 0
...
export default function GroupsNotification({ isDisable, allGroupsStatus }) {
    ...
    return (
        <div>
            <Button activeColor={activeColor} handleClick={handleClick} isDisable={isDisable} showNotification={showNotification} />
            {showNotification && <Modal handleClose={handleClose} notificationPosition={notificationPosition} statusCounts={statusCounts} />}
        </div>
    );
};